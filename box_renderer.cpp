...
void BoxRenderer::DrawBox(glm::vec3 position, glm::vec3 size, glm::vec3 color, Game& game)
{
    ...
    // set view pos 
    this->shader.SetVector3f("viewPos",game.player.camera.position.x, game.player.camera.position.y, game.player.camera.position.z);
    ...
    glm::mat4 view;
    view = game.player.camera.GetViewMatrix();
    glm::mat4 projection = glm::perspective(game.player.camera.zoom, (GLfloat)game.Width / (GLfloat)game.Height, 0.1f, 100.0f);
