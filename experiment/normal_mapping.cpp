...
int main()
{
    ...
    ResourceManager::LoadShader("./normal_mapping.vs", "./normal_mapping.fs", nullptr, "normal_mapping");
    Shader shader = ResourceManager::GetShader("normal_mapping");
    // Load textures
    GLuint diffuseMap = loadTexture("./brickwall.jpg");
    GLuint normalMap = loadTexture("./brickwall_normal.jpg");
    
    // GLuint diffuseMap = loadTexture("./156.png");
    
    // GLuint normalMap = loadTexture("./156_norm.png");
    // Set texture units 
    shader.Use();
    shader.SetInteger("diffuseMap", 0);
    shader.SetInteger("normalMap",1);
    // Light position
    glm::vec3 lightPos(0.5f, 1.0f, 5.0f);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shader.SetVector3f("viewPos",camera.Position.x,camera.Position.y, camera.Position.z);
...
void RenderQuad(GLfloat size)
{
    ...
    {
        // positions
        glm::vec3 pos1(-size, size, 0.0);
        glm::vec3 pos2(-size, -size, 0.0);
        glm::vec3 pos3(size, -size, 0.0);
        glm::vec3 pos4(size, size, 0.0);
        ...
        // texture coordinates
        glm::vec2 uv1(0.0, size);
        glm::vec2 uv2(0.0, 0.0);
        glm::vec2 uv3(size, 0.0);
        glm::vec2 uv4(size, size);
        // normal vector
        glm::vec3 nm(0.0, 0.0, size);
